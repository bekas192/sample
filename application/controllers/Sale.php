...
class Sale extends CI_Controller {
    ...
    function form_penjualan_autocomplit() {
        ...
        $sql_barang = "SELECT * FROM `p_item` WHERE barcode='$barcode' ";
    ...
    function keranjang() {
        $jumlah    = $_GET['jumlah'];
        $barcode = $_GET['barcode'];
        $chek      = $this->db->get_where('v_sale', array('barcode' => $barcode));
        if ($chek->num_rows()>0) {
           $sql="update v_sale set total_beli=total_beli+$jumlah where barcode='$kd_barang' ";
           $this->db->query($sql);
        }
        else{
            $id = $this->sale_m->insert($jumlah);
            $data = array(
                'barcode' => $barcode,
                'invoice' => $id,
            );
    ...
    function load_temp() {
        ...
        $data = $this->sale_m->show_temp()->result();
        $no = 1;
        $total = 0;
        foreach ($data as $d) {
            echo "<tr>"
